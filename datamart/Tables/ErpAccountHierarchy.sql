...
-- self-joining erp_account on code = parent_level_N_code (the source exposes parent codes only).
-- Joined by usp_GetGlBalanceSummary and usp_GetGlBalanceSummaryFilterOptions to
-- GlSummaryBalances.Account for hierarchy-aware filtering. Populated out-of-band for now; an ADF
-- pipeline will maintain it later.
create table dbo.ErpAccountHierarchy
(
    Code             varchar(20)    not null,  -- leaf natural-account value; join key to GlSummaryBalances.Account
