...
BEGIN
    ...
    SET @RedshiftQuery = '
        WITH gl_summary AS (
            ...
            WHERE financial_department = ''' + @FinancialDept + ''''
            + CASE WHEN @ExcludedDocNumbers IS NOT NULL
                   THEN ' AND DOCUMENT_NUMBER NOT IN (' + @ExcludedDocNumbers + ')'
                   ELSE ''
              END + '
            GROUP BY PROJECT, PROJECT_DESCRIPTION, FUND, FUND_DESCRIPTION, PROGRAM, PROGRAM_DESCRIPTION, ACTIVITY, ACTIVITY_DESCRIPTION
        ),
        ppm_summary AS (
            SELECT
                PROJECT_NUMBER,
            ...
            WHERE prj_owning_cd = ''' + @FinancialDept + '''
            GROUP BY PROJECT_NUMBER, PROJECT_NAME, FUND_CD, PROGRAM_CD, ACTIVITY_CD
        )
        SELECT
        ...
        FULL OUTER JOIN ppm_summary ppm
            ON gl.PROJECT = ppm.PROJECT_NUMBER
